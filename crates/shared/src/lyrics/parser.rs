...
pub fn parse_lyrics(lyrics_text: &str) -> Vec<LyricLine> {
    ...
    while i < lines_vec.len() {
        ...
        if let Some(caps) = timestamp_regex.captures(line) {
            ...
            // 检查下一行是否是翻译
            // let translation = if i + 1 < lines_vec.len() {
            //     let next_line = lines_vec[i + 1];
            //     // 如果下一行不是时间戳行且不为空，则认为是翻译
            //     if !timestamp_regex.is_match(next_line)
            //         && !next_line.trim().is_empty()
            //         && !next_line.starts_with('[')
            //     {
            //         i += 1; // 跳过翻译行
            //         Some(next_line.trim().to_string())
            //     } else {
            //         None
            //     }
            // } else {
            ...
            if !clean_text.is_empty() {
                ...
                //     lines.push(LyricLine {
                //         timestamp: timestamp + 0.1, // 稍微延后显示翻译
                //         text: trans,
                //         words: vec![],
                //     });
                // }
